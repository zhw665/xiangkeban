...
- 移除应用启动时自动执行迁移的逻辑。迁移由 Netlify lifecycle 管理，演示数据通过显式、幂等的 seed 命令创建。
- seed 命令只在演示学校不存在时插入青禾中心小学、五年级一班及教师、学生、家长演示数据，不覆盖真实数据。
...
- 单元测试覆盖生产配置校验、教师邀请码、家长绑定码摘要/过期/单次使用和演示 seed 幂等性。
