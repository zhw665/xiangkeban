...
export async function POST(request: Request) {
  ...
  const existingUsers = await db.select({ id: users.id }).from(users).where(eq(users.username, input.username));
  ...
  if (input.role === "student") {
    ...
    targetClass = firstOrNull(await db.select().from(classes).where(eq(classes.inviteCode, input.inviteCode.trim().toUpperCase()))) ?? undefined;
  ...
  if (input.role === "parent") {
    ...
    linkedStudent = firstOrNull(await db.select().from(users).where(eq(users.username, input.studentUsername.trim()))) ?? undefined;
  ...
  await db.transaction(async (tx) => {
    await tx.insert(users).values({ id: userId, schoolId, username: input.username, name: input.name, passwordHash, role: input.role, avatarColor: input.role === "student" ? "blue" : input.role === "parent" ? "amber" : "green", createdAt: now });
    if (input.role === "teacher") {
      generatedInviteCode = `QH${randomBytes(3).toString("hex").toUpperCase()}`;
      const classId = randomUUID();
      await tx.insert(classes).values({ id: classId, schoolId, teacherId: userId, name: input.className!, grade: input.grade!, inviteCode: generatedInviteCode, createdAt: now });
      await tx.insert(classMembers).values({ classId, userId, memberRole: "teacher", joinedAt: now });
    } else if (input.role === "student" && targetClass) {
      await tx.insert(classMembers).values({ classId: targetClass.id, userId, memberRole: "student", joinedAt: now });
    } else if (input.role === "parent" && linkedStudent) {
      await tx.insert(guardianLinks).values({ guardianId: userId, studentId: linkedStudent.id, relation: input.relation || "监护人", createdAt: now });
    }
  });
