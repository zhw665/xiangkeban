...
export async function POST(request: Request) {
  ...
  if (attachment) {
    ...
    await db.insert(files).values({ id: fileId, schoolId: session.user.schoolId, classId: classroom.id, ownerId: session.user.id, name: attachment.name, mimeType: attachment.type, size: attachment.size, storageKey, createdAt: now }).run();
  }
  const students = await db.select().from(classMembers).where(eq(classMembers.classId, classroom.id)).all();
  await db.transaction(async (tx) => {
    await tx.insert(assignments).values({ id, classId: classroom.id, teacherId: session.user.id, fileId, title: parsed.data.title, subject: parsed.data.subject, description: parsed.data.description, dueAt: parsed.data.dueAt, status: "published", createdAt: now }).run();
    await tx.insert(assignmentItems).values(parsed.data.items.map((item, orderNo) => ({ id: randomUUID(), assignmentId: id, prompt: item.prompt, type: item.type, optionsJson: JSON.stringify(item.options), answer: item.answer, points: item.points, orderNo }))).run();
    const studentIds = students.filter((item) => item.memberRole === "student").map((item) => item.userId);
    if (studentIds.length) await tx.insert(notifications).values(studentIds.map((userId) => ({ id: randomUUID(), userId, title: "有新作业", body: parsed.data.title, type: "assignment", href: "/student/assignments", readAt: null, createdAt: now }))).run();
  });
