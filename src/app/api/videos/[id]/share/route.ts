...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  await dbReady;
  const video = await db.select().from(videos).where(and(eq(videos.id, id), eq(videos.classId, classroom.id))).get();
  if (!video) return jsonError("微课不存在", 404);
  const existing = await db.select().from(videoShares).where(and(eq(videoShares.videoId, id), eq(videoShares.targetClassName, parsed.data.targetClassName))).get();
  if (!existing) await db.insert(videoShares).values({ id: randomUUID(), videoId: id, teacherId: session.user.id, targetClassName: parsed.data.targetClassName, createdAt: nowIso() }).run();
